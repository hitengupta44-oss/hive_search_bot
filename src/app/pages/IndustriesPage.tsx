...
export function IndustriesPage() {
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero Section */}
      <section className="relative min-h-[450px] md:h-[500px] overflow-hidden mt-[80px] md:mt-20 flex items-center py-12 md:py-0">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {industries.map((ind, idx) => (
              ...
              >
                ...
                <div className="mt-8 pt-6 border-t border-gray-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex flex-wrap gap-2">
                    {ind.results.slice(0, 1).map((r, i) => (
                      <div key={i} className="px-3 py-1.5 bg-emerald-50 rounded-lg text-xs font-black text-emerald-600 uppercase tracking-widest shrink-0">
                        {r}
                      </div>
                    ))}
                  </div>
                  <Link 
                    to="/contact" 
                    className="flex items-center gap-2 text-emerald-600 font-black text-sm uppercase tracking-widest hover:gap-3 transition-all mt-2 sm:mt-0"
                  >
